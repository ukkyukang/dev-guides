...
- [🎯 학습 목표](#학습-목표)
- [📖 본문](#본문)
  - [1. Python 패키지란?](#1-python-패키지란)
  - [2. 현대적 패키지 구조: `pyproject.toml`](#2-현대적-패키지-구조-pyprojecttoml)
    - [2.1 핵심 표준 (PEP)](#21-핵심-표준-pep)
    - [2.2 `pyproject.toml` 전체 구조](#22-pyprojecttoml-전체-구조)
    - [2.3 각 섹션 상세 설명](#23-각-섹션-상세-설명)
  - [3. 프로젝트 레이아웃](#3-프로젝트-레이아웃)
    - [3.1 Src Layout (권장 ✅)](#31-src-layout-권장)
    - [3.2 Flat Layout](#32-flat-layout)
    - [3.3 어떤 레이아웃을 선택해야 하나?](#33-어떤-레이아웃을-선택해야-하나)
  - [4. 빌드 시스템 이해](#4-빌드-시스템-이해)
    - [4.1 주요 빌드 백엔드 비교](#41-주요-빌드-백엔드-비교)
    - [4.2 빌드 결과물](#42-빌드-결과물)
    - [4.3 `[build-system]` 설정 예시](#43-build-system-설정-예시)
  - [5. 네임스페이스 패키지](#5-네임스페이스-패키지)
    - [5.1 왜 필요한가?](#51-왜-필요한가)
    - [5.2 구현 방법 (Implicit Namespace Package)](#52-구현-방법-implicit-namespace-package)
    - [5.3 사내 네임스페이스 패키지 관례](#53-사내-네임스페이스-패키지-관례)
  - [6. 패키지 배포](#6-패키지-배포)
    - [6.1 PyPI에 배포](#61-pypi에-배포)
    - [6.2 사설 인덱스에 배포](#62-사설-인덱스에-배포)
    - [6.3 사설 인덱스에서 설치](#63-사설-인덱스에서-설치)
    - [6.4 버전 관리 전략](#64-버전-관리-전략)
  - [7. 패키지 구조 체크리스트](#7-패키지-구조-체크리스트)
    - [필수 항목](#필수-항목)
    - [권장 항목](#권장-항목)
  - [8. `setup.py`에서 `pyproject.toml`로 마이그레이션](#8-setuppy에서-pyprojecttoml로-마이그레이션)
    - [8.1 `setup.py` → `pyproject.toml` 대응](#81-setuppy-pyprojecttoml-대응)
    - [8.2 마이그레이션 단계](#82-마이그레이션-단계)
- [❓ 자주 묻는 질문 (FAQ)](#자주-묻는-질문-faq)
  - [Q1: `name`에 하이픈(`-`)과 언더스코어(`_`) 중 어떤 것을 써야 하나요?](#q1-name에-하이픈-과-언더스코어_-중-어떤-것을-써야-하나요)
  - [Q2: `[project.optional-dependencies]`와 `[dependency-groups]`의 차이는?](#q2-projectoptional-dependencies와-dependency-groups의-차이는)
  - [Q3: `__init__.py`에 무슨 코드를 넣어야 하나요?](#q3-__init__py에-무슨-코드를-넣어야-하나요)
  - [Q4: `py.typed`는 무엇이고 왜 필요한가요?](#q4-pytyped는-무엇이고-왜-필요한가요)
  - [Q5: wheel과 sdist의 차이는?](#q5-wheel과-sdist의-차이는)
  - [Q6: `hatchling`을 선택한 이유는?](#q6-hatchling을-선택한-이유는)
  - [Q7: 패키지 이름이 다른 패키지와 충돌하면 어떻게 되나요?](#q7-패키지-이름이-다른-패키지와-충돌하면-어떻게-되나요)
- [🏗️ 예제 프로젝트](#예제-프로젝트)
- [🔗 참고 자료](#참고-자료)
- [⏭️ 다음 강의](#다음-강의)
