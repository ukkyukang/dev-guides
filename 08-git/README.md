...
- [📖 본문](#본문)
  - [1. Git이란?](#1-git이란)
    - [1.1 핵심 개념](#11-핵심-개념)
    - [1.2 Git의 3가지 영역](#12-git의-3가지-영역)
  - [2. Git 설치 및 초기 설정](#2-git-설치-및-초기-설정)
  - [3. 🗺️ 실무 개발 워크플로우 전체 순서도](#3-실무-개발-워크플로우-전체-순서도)
    - [3.1 요약: 한 줄로 보는 전체 흐름](#31-요약-한-줄로-보는-전체-흐름)
    - [3.2 브랜치 구조 시각화](#32-브랜치-구조-시각화)
  - [4. 기본 명령어 상세 설명](#4-기본-명령어-상세-설명)
    - [4.1 저장소 초기화](#41-저장소-초기화)
    - [4.2 변경사항 스테이징 & 커밋](#42-변경사항-스테이징-커밋)
    - [4.3 커밋 메시지 컨벤션](#43-커밋-메시지-컨벤션)
    - [4.4 이력 확인](#44-이력-확인)
    - [4.5 원격 저장소 (Remote)](#45-원격-저장소-remote)
  - [5. 브랜치](#5-브랜치)
    - [5.1 브랜치 기본 명령어](#51-브랜치-기본-명령어)
    - [5.2 브랜치 네이밍 컨벤션](#52-브랜치-네이밍-컨벤션)
  - [6. 머지 (Merge)](#6-머지-merge)
    - [6.1 기본 머지](#61-기본-머지)
    - [6.2 Merge vs Rebase](#62-merge-vs-rebase)
    - [6.3 머지 충돌 해결](#63-머지-충돌-해결)
  - [7. 태그 (Tag)](#7-태그-tag)
    - [7.1 Semantic Versioning (SemVer)](#71-semantic-versioning-semver)
  - [8. `.gitignore`](#8-gitignore)
  - [9. 명령어 치트시트](#9-명령어-치트시트)
- [❓ 초보자가 겪는 30가지 상황별 해결법 (FAQ)](#초보자가-겪는-30가지-상황별-해결법-faq)
  - [🔥 커밋 실수](#커밋-실수)
    - [Q1: 방금 커밋 메시지를 잘못 썼어요](#q1-방금-커밋-메시지를-잘못-썼어요)
    - [Q2: 커밋에 파일을 빼먹었어요](#q2-커밋에-파일을-빼먹었어요)
    - [Q3: 방금 커밋을 취소하고 싶어요 (코드는 유지)](#q3-방금-커밋을-취소하고-싶어요-코드는-유지)
    - [Q4: 방금 커밋을 완전히 삭제하고 싶어요](#q4-방금-커밋을-완전히-삭제하고-싶어요)
    - [Q5: 이미 push한 커밋을 취소하고 싶어요](#q5-이미-push한-커밋을-취소하고-싶어요)
  - [🔥 파일 수정 실수](#파일-수정-실수)
    - [Q6: 수정한 파일을 원래대로 되돌리고 싶어요 (커밋 전)](#q6-수정한-파일을-원래대로-되돌리고-싶어요-커밋-전)
    - [Q7: `git add`한 것을 취소하고 싶어요 (스테이징 해제)](#q7-git-add한-것을-취소하고-싶어요-스테이징-해제)
    - [Q8: 삭제한 파일을 복구하고 싶어요](#q8-삭제한-파일을-복구하고-싶어요)
    - [Q9: 특정 커밋의 파일 상태로 되돌리고 싶어요](#q9-특정-커밋의-파일-상태로-되돌리고-싶어요)
    - [Q10: `.gitignore`에 추가했는데 파일이 계속 추적돼요](#q10-gitignore에-추가했는데-파일이-계속-추적돼요)
  - [🔥 브랜치 문제](#브랜치-문제)
    - [Q11: 현재 어떤 브랜치에 있는지 모르겠어요](#q11-현재-어떤-브랜치에-있는지-모르겠어요)
    - [Q12: 브랜치를 만들고 안 옮겨갔어요](#q12-브랜치를-만들고-안-옮겨갔어요)
    - [Q13: 잘못된 브랜치에서 작업했어요 (아직 커밋 안 함)](#q13-잘못된-브랜치에서-작업했어요-아직-커밋-안-함)
    - [Q14: 잘못된 브랜치에 커밋해버렸어요](#q14-잘못된-브랜치에-커밋해버렸어요)
    - [Q15: 원격 브랜치를 로컬로 가져오고 싶어요](#q15-원격-브랜치를-로컬로-가져오고-싶어요)
    - [Q16: 브랜치를 삭제하려는데 에러가 나요](#q16-브랜치를-삭제하려는데-에러가-나요)
  - [🔥 머지 & 충돌](#머지-충돌)
    - [Q17: merge 충돌이 났어요! 어떻게 해요?](#q17-merge-충돌이-났어요-어떻게-해요)
    - [Q18: 머지를 취소하고 싶어요 (충돌 해결 중)](#q18-머지를-취소하고-싶어요-충돌-해결-중)
    - [Q19: 머지했는데 되돌리고 싶어요 (이미 커밋됨)](#q19-머지했는데-되돌리고-싶어요-이미-커밋됨)
    - [Q20: dev 브랜치의 최신 내용을 내 feature 브랜치에 반영하고 싶어요](#q20-dev-브랜치의-최신-내용을-내-feature-브랜치에-반영하고-싶어요)
  - [🔥 push & pull 문제](#push-pull-문제)
    - [Q21: `git push`가 rejected 됐어요](#q21-git-push가-rejected-됐어요)
    - [Q22: `git pull`하니까 머지 커밋이 자꾸 생겨요](#q22-git-pull하니까-머지-커밋이-자꾸-생겨요)
    - [Q23: 원격 저장소 URL을 변경하고 싶어요](#q23-원격-저장소-url을-변경하고-싶어요)
  - [🔥 되돌리기 & 복구](#되돌리기-복구)
    - [Q24: 특정 커밋으로 돌아가서 코드를 보고 싶어요](#q24-특정-커밋으로-돌아가서-코드를-보고-싶어요)
    - [Q25: `reset --hard`를 잘못 써서 코드를 날려버렸어요!](#q25-reset---hard를-잘못-써서-코드를-날려버렸어요)
    - [Q26: 특정 커밋의 변경사항만 가져오고 싶어요](#q26-특정-커밋의-변경사항만-가져오고-싶어요)
  - [🔥 임시 저장 (Stash)](#임시-저장-stash)
    - [Q27: 작업 중인데 급하게 다른 브랜치로 가야 해요](#q27-작업-중인데-급하게-다른-브랜치로-가야-해요)
    - [Q28: stash가 여러 개 쌓였어요](#q28-stash가-여러-개-쌓였어요)
  - [🔥 기타 자주 겪는 상황](#기타-자주-겪는-상황)
    - [Q29: 대용량 파일을 실수로 커밋해버렸어요 (100MB+)](#q29-대용량-파일을-실수로-커밋해버렸어요-100mb)
    - [Q30: 커밋 이력에서 민감한 정보(API 키 등)를 완전히 삭제하고 싶어요](#q30-커밋-이력에서-민감한-정보api-키-등를-완전히-삭제하고-싶어요)
- [📊 명령어 빈도 순위 (실무 기준)](#명령어-빈도-순위-실무-기준)
- [🔗 참고 자료](#참고-자료)
- [⏭️ 학습 순서](#학습-순서)
